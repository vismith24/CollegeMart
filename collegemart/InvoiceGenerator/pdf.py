...
class SimpleInvoice(BaseInvoice):
    ...
    def _drawTitle(self):
        ...
        if not self.invoice.use_tax:
            self.pdf.drawString(
                (self.LEFT + 90) * mm,
                self.TOP*mm,
                _(u'Order ID: %s') % self.invoice.number,
            )
        else:
            self.pdf.drawString(
                (self.LEFT + 90) * mm,
    ...
    def _drawProvider(self, TOP, LEFT):
        self._drawAddress(TOP, LEFT, 88, 36, _(u'Seller'), self.invoice.provider)
    ...
    def _drawPayment(self, TOP, LEFT):
        ...
        lines = [
            '%s: %s' % (_(u'Method'), self.invoice.provider.payment_method),
        ]
    ...
    def _drawItems(self, TOP, LEFT):  # noqa
        ...
        for item in self.invoice.items:
            ...
            if items_are_with_tax:
                ...
                if float(int(item.count)) == item.count:
                    self.pdf.drawRightString((LEFT + 110) * mm, (TOP - i) * mm, u'%s %s' % (locale.format("%i", item.count, grouping=True), item.unit))
                else:
                    self.pdf.drawRightString((LEFT + 110) * mm, (TOP - i) * mm, u'%s %s' % (locale.format("%.2f", item.count, grouping=True), item.unit))
                self.pdf.drawRightString((LEFT + 132) * mm, (TOP - i) * mm, currency(item.price, self.invoice.currency, self.invoice.currency_locale))
                self.pdf.drawRightString((LEFT + 163) * mm, (TOP - i) * mm, currency(item.total, self.invoice.currency, self.invoice.currency_locale))
                i += 5
        ...
        if not items_are_with_tax:
            ...
            self.pdf.drawString((LEFT + 130) * mm, (TOP - i - 7) * mm, '%s: %s' % (_(u'Total'), currency(self.invoice.price, self.invoice.currency, self.invoice.currency_locale)))
        else:
    ...
    def _drawCreator(self, TOP, LEFT):
        ...
        self.pdf.drawString((LEFT + 15) * mm, (TOP - 5) * mm - height, '     %s %s' % (_(u'Sd/-'), self.invoice.creator.name))
